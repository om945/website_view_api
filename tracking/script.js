(function () {
  ...
  try {
    ...
    var heartbeatMs = 15000, log = function () { if (debug && console && console.log) console.log.apply(console, arguments); };
    ...
    function connectPresence() { if (stopped || socket && (socket.readyState === 0 || socket.readyState === 1)) return; if (testState) testState.connectCalls++; try { socket = new WebSocket(base.replace(/^http/, "ws") + "/ws/track"); socket.onopen = function () { reconnectAttempt = 0; socket.send(presenceMessage()); log("Realtime status: connected"); }; socket.onclose = function () { socket = null; if (heartbeatTimer) { clearInterval(heartbeatTimer); heartbeatTimer = null; if (testState) { testState.heartbeatClears++; testState.heartbeatActive = false; } } scheduleReconnect(); }; socket.onerror = function () { log("Realtime status: error"); }; if (!heartbeatTimer) { heartbeatTimer = setInterval(function () { if (socket && socket.readyState === 1) socket.send(presenceMessage()); }, heartbeatMs); if (testState) { testState.heartbeatStarts++; testState.heartbeatActive = true; } } } catch (_) { scheduleReconnect(); } }
    function page() { try { var u = new URL(location.href), q = u.searchParams, body = { siteKey: site, visitorId: visitorId, eventId: crypto.randomUUID ? crypto.randomUUID() : String(Date.now()) + Math.random(), path: u.pathname + u.search, fullUrl: u.href, referrer: document.referrer || null, language: navigator.language, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || null, screenWidth: screen.width, screenHeight: screen.height, utm: { source: q.get("utm_source"), medium: q.get("utm_medium"), campaign: q.get("utm_campaign") } }; fetch(base + "/api/v1/track", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body), keepalive: true }).catch(function (e) { log("Tracking unavailable", e); }); } catch (_) {} }
    function track(name, properties) { try { if (typeof name !== "string" || !name || name.length > 100) return; fetch(base + "/api/v1/events", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ siteKey: site, visitorId: visitorId, name: name, properties: properties || {} }), keepalive: true }).catch(function (e) { log("Event unavailable", e); }); } catch (_) {} }
